...
> 
> Go 1.20 reached End of Life and is no longer receiving updates.
> 
...
Debian packaging for Go 1.20: compiles the Go toolchain from the official [golang/go](https://github.com/golang/go) source tree into `golang-1.20-go` packages for enterprise `.deb`-only installs.
...
```bash
make materialize GO=1.20 DIST=trixie
make build GO=1.20
```
...
| Path | Purpose |
|------|---------|
| `go/` | Go source tree (git submodule from golang/go, `release-branch.go1.20` branch) |
